...
import { Flame, Sun, Moon, CloudRain, Volume2, VolumeX, BookOpen, CircleHelp, Compass, Pause, Play, Footprints, Orbit, Crosshair, Sparkles, Users, Sunrise, X, MessageCircle, Brain, Send, ArrowUp, ArrowDown, ArrowLeft, ArrowRight, Leaf, MousePointer2, Check, Save } from 'lucide-react';
import { Sheet, SheetContent, SheetHeader, SheetTitle, SheetDescription } from '@/components/ui/sheet';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription } from '@/components/ui/dialog';
import { Tabs, TabsList, TabsTrigger, TabsContent } from '@/components/ui/tabs';
import { Progress } from '@/components/ui/progress';
import { DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuLabel, DropdownMenuItem } from '@/components/ui/dropdown-menu';
import { Simulation, clockLabel, type Agent } from './simulation';
...
export default function WorldApp(){
  ...
  useEffect(()=>{
    ...
    import('./scene').then(({WorldScene})=>{
      ...
      try{instance=new WorldScene(canvas.current,simulation,{onSelect:choose,onReady:()=>setReady(true),onError:setError,onFrame:()=>render(n=>n+1)});world.current=instance;if(restored)notify('Welcome back. Your world remembers.');autosave=setInterval(()=>setSaved(simulation.save()),10000);}
  ...
  useEffect(()=>{const onKey=(e:KeyboardEvent)=>{if((e.target as HTMLElement)?.closest('input,textarea,[role="dialog"]'))return;if(e.key===' '){e.preventDefault();setPaused(p=>{if(world.current)world.current.paused=!p;return !p;});}if(e.key.toLowerCase()==='j')setJournal(p=>!p);if(e.key==='Escape'){choose(null);if(world.current?.mode==='walk'){world.current.setMode('orbit');setMode('orbit');}}};window.addEventListener('keydown',onKey);return()=>window.removeEventListener('keydown',onKey);},[choose]);
  const s=sim.current;
  ...
  return <main className="world-app">
    ...
    {!agent&&<div className="compass" aria-hidden="true"><div className="compass-ring"><Compass strokeWidth={1}/></div><span>N &nbsp; · &nbsp; E</span></div>}

    <Sheet open={!!agent&&!journal&&!help} modal={false} onOpenChange={open=>{if(!open)choose(null);}}>
    ...
    <span className="world-badge"><Leaf/> Autonomous simulation</span>
    <span className="explore-tip">{mode==='walk'?<><Footprints size={13}/> Drag to look · arrows to walk</>:<><MousePointer2 size={13}/> Drag to explore · select an inhabitant</>}</span>
    ...
    <Dialog open={help} onOpenChange={setHelp}><DialogContent className="help-dialog"><DialogHeader><DialogTitle>A little world. A life of its own.</DialogTitle><DialogDescription className="help-note">Take your time. The inhabitants will carry on around you.</DialogDescription></DialogHeader><div className="help-grid"><div><Orbit/><strong>Explore the clearing</strong><p>Drag to orbit. Scroll or pinch to zoom. Use Walk and WASD or the touch arrows to enter the world.</p></div><div><MessageCircle/><strong>Meet someone</strong><p>Select an inhabitant. Ask about their day, inspect a memory, or follow them along the trail.</p></div><div><Sparkles/><strong>Change their day</strong><p>Start a gathering, bring in rain, or skip to dusk. Watch their plans respond.</p></div><div><Brain/><strong>Watch connections grow</strong><p>Encounters become memories. Conversations build friendship. Their needs guide their next task.</p></div></div><p className="help-note">This is a browser simulation with goal-based agents and contextual dialogue. A live language model is not connected. Your world saves locally on this device and pauses while the tab is hidden.</p><p className="help-note">Space: pause · J: journal · Escape: leave walking mode<br/>Sound starts muted. Enable the speaker for forest ambience.</p><button className="help-main" onClick={()=>setHelp(false)}>Back to the clearing</button></DialogContent></Dialog>
